...
class CombinedVisualLoss(nn.Module):
    def __init__(self, config: Optional[CombinedVisualLossConfig] = None):
        ...
        self.config = config \
                        ...
                        else CombinedVisualLossConfig()
        
        self.log_weights = nn.Parameter(
            torch.log(torch.tensor([
                config.init_dssim_weight,
                config.init_l2_weight,
                config.init_perceptual_weight
            ])),
    ...
    def _build_ssim_kernel(self):
        ...
        cfg = self.config
        x = torch.linspace(-cfg.ssim_window_size // 2, cfg.ssim_window_size // 2, cfg.ssim_window_size)
        gauss = torch.exp(-x.pow(2) / (2 * cfg.ssim_sigma ** 2))
        gauss = gauss / gauss.sum()
        kernel = gauss.unsqueeze(0).unsqueeze(0)
    ...
    def _build_perceptual_layers(self):
        ...
        # Map layer names to feature indices
        layer_map = {
            'relu1_2': 4,
            'relu2_2': 9,
            'relu3_4': 18,
        ...
        self.perceptual_mean = torch.tensor(cfg.perceptual_mean).view(1, 3, 1, 1)
        self.perceptual_std = torch.tensor(cfg.perceptual_std).view(1, 3, 1, 1)
    ...
    def _ssim_map(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        print(x.shape, self.ssim_kernel.shape)
        mu_x = F.conv2d(x, self.ssim_kernel, padding=cfg.ssim_window_size // 2, groups=x.size(1))
        mu_y = F.conv2d(y, self.ssim_kernel, padding=cfg.ssim_window_size // 2, groups=y.size(1))
        
        mu_x_sq = mu_x.pow(2)
        mu_y_sq = mu_y.pow(2)
        mu_xy = mu_x * mu_y
        
        sigma_x_sq = F.conv2d(x * x, self.ssim_kernel, padding=cfg.ssim_window_size // 2, groups=x.size(1)) - mu_x_sq
        sigma_y_sq = F.conv2d(y * y, self.ssim_kernel, padding=cfg.ssim_window_size // 2, groups=y.size(1)) - mu_y_sq
        sigma_xy = F.conv2d(x * y, self.ssim_kernel, padding=cfg.ssim_window_size // 2, groups=x.size(1)) - mu_xy
        
        ssim_map = ((2 * mu_xy + cfg.ssim_c1)           \
                    * (2 * sigma_xy + cfg.ssim_c2)) /   \
    ...
    def _perceptual_loss(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        for layer in self.perceptual_layers:
            ...
            if self.perceptual_use_l1:
                total_loss += F.l1_loss(x_norm, y_norm)
            else:
                total_loss += F.mse_loss(x_norm, y_norm)
        
        return total_loss / len(self.perceptual_layers)
    ...
    def get_difference_maps(self, pred: torch.Tensor, gt: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        for layer in self.perceptual_layers:
            ...
            gt_norm = layer(gt_norm)
            pred_features.append(pred_norm)
            gt_features.append(gt_norm)
